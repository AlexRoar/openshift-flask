...
def getSressWeb(word):
    if len(proxy) > 0:
        proxyDict = proxy[random.randint(0, len(proxy) - 1)]
...
def handle(req, res):
    ...
    if req['request']['original_utterance'].lower() == 'stat_31415926':
        ...
        r = c.execute('SELECT * FROM `not_found` ORDER BY count DESC')
        r = r.fetchall()
        for i in r:
    ...
    if user_text.lower() == 'помощь' or user_text.lower() == 'что ты умеешь' or user_text.lower() == 'что ты умеешь?':
        res['response']['text'] = 'Я помогу тебе узнать правильное ударение в частоошибаемых русских словах.' \
                                  ...
                                  'я буду предлагать варианты слов, а тебе нужно будет выбрать правильное'
    ...
    elif (user_text.lower() == 'ещё' or user_text.lower() == 'еще') and session['action'] == 'game':
        res['response']['text'] = 'Выбери верное'
...
def responseForGame(res, user_id, user_text, session):
    if (user_text == session['right']):
        ...
        res['response']['tts'] = randomWin() + 'Верно!'
    else:
        ...
        res['response']['text'] = 'Неверно! Правильно так: ' + session['right']
        res['response']['tts'] = randomFail() + 'Неверно! - - - Правильно так: ' + ttsWordCapital(session['right'])
...
def notFoundWord(word):
    res = c.execute('SELECT * FROM `not_found` WHERE `word`="' + word.lower() + '"')
    fetch = res.fetchall()
    if (len(fetch) == 0):
        c.execute('INSERT INTO `not_found` VALUES ("' + word.lower() + '", "1")')
        connection.commit()
        return
    res = fetch[0]
    count = int(res[-1]) + 1
    c.execute("UPDATE `not_found` SET 'count'=" + str(count) + ' WHERE `word`="' + word.lower() + '"')
    connection.commit()


def statChoice(choice):
    res = c.execute('SELECT * FROM `words_choice` WHERE `choice`="' + choice + '"')
    fetch = res.fetchall()
    if (len(fetch) == 0):
        c.execute('INSERT INTO `words_choice` VALUES ("' + choice.lower() + '", "' + choice + '", 1)')
        connection.commit()
        return
    res = fetch[0]
    count = int(res[-1]) + 1
    c.execute("UPDATE `words_choice` SET 'count'=" + str(count) + ' WHERE `choice`="' + choice + '"')
    connection.commit()
